...
describe('consensus message', () => {
  ...
  beforeAll(() => {
    ...
    block = Block.construct(height, epoch, lastBlockHash, state, transactions, consensus, validatorSet)
  ...
  it('destruct with match', () => {
    ...
    expect(proposal.match(p => p , _ => null, _ => null)).toEqual(new Proposal(1, 1, 0, block))
    expect(preVote.match(_ => null , v => v, _ => null)).toEqual(new Vote(1, 1, block.hash))
    expect(preCommit.match(_ => null , _ => null, v => v)).toEqual(new Vote(1, 2, block.hash))
  })
