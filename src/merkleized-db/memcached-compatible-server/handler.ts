...
const logger = debug('state-db:memcached-protocol')
...
export interface MemcachedSubset {
  set (key: string, flags: number, data: Buffer): Promise<Response.Stored>
  add (key: string, flags: number, data: Buffer): Promise<Response.Stored | Response.NotStored>
  replace (key: string, flags: number, data: Buffer): Promise<Response.Stored | Response.NotStored>
  append (key: string, data: Buffer): Promise<Response.Stored | Response.NotStored>
  prepend (key: string, data: Buffer): Promise<Response.Stored | Response.NotStored>
  cas (key: string, flags: number, cas: number, data: Buffer): Promise<Response.Stored | Response.Exists | Response.NotFound>
  get (keys: string[]): AsyncIterable<Response.Value>
  gets (keys: string[]): AsyncIterable<Response.ValueS>
  delete (key: string): Promise<Response.Deleted | Response.NotFound>
  incr (key: string, value: number): Promise<Response.NotFound | Response.Number>
  decr (key: string, value: number): Promise<Response.NotFound | Response.Number>
  stats (...args: any[]): AsyncIterable<Response.Stat>
  flush (): Promise<Response.Ok>
  version (): Promise<Response.Version>
  verbosity (level: number): Promise<Response.Ok>
  quit (): Promise<void>
}
